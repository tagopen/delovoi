<?php
  ...
  if ( (!empty($_POST["phone"])) && (isset($_POST["phone"])) ) {
    $post['user_phone'] = filter_input(INPUT_POST, 'phone', FILTER_SANITIZE_EMAIL);
    $body .= 'Телефон: ' . $post['user_phone'] . chr(10) . chr(13);
  }

  if ( (!empty($_POST["name"])) && (isset($_POST["name"])) ) {
    $post['user_name'] = filter_input(INPUT_POST, 'name', FILTER_SANITIZE_EMAIL);
    $body .= 'Имя: ' . $post['user_name'] . chr(10) . chr(13);
  }

  if ( (!empty($_POST["message"])) && (isset($_POST["message"])) ) {
    $post['user_message'] = filter_input(INPUT_POST, 'message', FILTER_SANITIZE_EMAIL);
    $body .= 'Сообщение: ' . $post['user_message'] . chr(10) . chr(13);
  }

  if ( (!empty($_POST["time"])) && (isset($_POST["time"])) ) {
    $post['user_time'] = filter_input(INPUT_POST, 'time', FILTER_SANITIZE_EMAIL);
